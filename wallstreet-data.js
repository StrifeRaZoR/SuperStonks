// Edit import line to include absolute path. e.g. '/stocks/wallstreet.js'
  ...
  while (true) {
    ...
    try {
      ...
      await channel3.write(ticker)
        ...
        topText.innerHTML = 'FORECAST: ' + Math.floor((ns.stock.getForecast(ns.sprintf(ticker))*100)) + '%' + ' || 5MIN HIGH: ' + ns.nFormat(channel1.peek(), '$0.00a') + ' || [LONG] PROFIT: ' + ns.nFormat(ns.stock.getSaleGain(ticker, position[0], "Long") - (position[0] * position[1]), '0.00a');
        HighlightText(topTextBG, topText, container)

        midText.innerHTML = '['+ ticker + ']' + ': ' + ns.nFormat(ns.stock.getPrice(ns.sprintf(ticker)), '$0.00a') + ' || ('+fiveminavg+' AVG)'
        HighlightText(midTextBG, midText, container)

        botText.innerHTML = 'VOL: ' + ns.nFormat(ns.stock.getVolatility(ns.sprintf(ticker)), '0.00%') + ' || 5MIN LOW: ' + ns.nFormat(channel2.peek(), '$0.00a') + ' || [SHORT] PROFIT: ' + ns.nFormat(ns.stock.getSaleGain(ticker, position[2], "Short") - (position[2] * position[3]), '0.00a');
        HighlightText(botTextBG, botText, container)
    ...
    if (ns.isRunning(scriptDir+"wallstreet.js", "home") == false) {
      await channel1.clear();
      await channel2.clear();
      await channel3.clear();
      ns.exit();
